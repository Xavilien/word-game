...
def get_scores(dictionary, player):
    ...
    for word in dictionary:
        parity = len(word) % 2
        if parity == 0 and player == 1:
            scores[word] = 1
        elif parity == 1 and player == 2:
            scores[word] = 1
        else:
            scores[word] = 0
    return scores
...
def get_possible(dictionary, curr_word):
    ...
    if len(possible_words) == 0 or possible_words[0] == "":
        return [], [""]
    next_possible_letters = list(set([i[0] for i in possible_words]))

    return list(set(next_possible_letters)), possible_words
